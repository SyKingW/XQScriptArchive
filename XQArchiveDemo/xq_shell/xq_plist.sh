...
function automaticAddBundle() {
    ...
    if [ ! -f $a_plist_path ]; then
        #跳出函数
        echo 'automaticAddBundle plist地址错误: $a_plist_path'
        exit 0
    fi
...
function configPlist() {
    ...
    if [ ! -f ${xq_plistPath} ]; then
        echo 'configExport plist地址错误: ' ${xq_plistPath}
        exit 0
    fi
    ...
    /usr/libexec/PlistBuddy -c "Set :provisioningProfiles:$xq_bundleID 6ce4cd76-1046-4ad8-8c4f-7112dd855499" $xq_plistPath
